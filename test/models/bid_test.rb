...
class BidTest < ActiveSupport::TestCase
  ...
  test "cannot bid after product expiration " do
    @bid = Bid.new(product: @product, user:users(:two), created_at: Time.now + 3.hours)
    assert !@bid.valid?, "cannot bid after expiration"
  end

  test "same user cannot bid two consecutive times" do
    @bid = Bid.create(user: users(:two), product: products(:one))
  ...
  test "same user can bid if another user bid after him" do
    @bid = Bid.create(user: users(:two), product: products(:one))
    @bid2 = Bid.create(user: users(:two), product: products(:one))
    @bid3 = Bid.create(user: users(:two), product: products(:one))
